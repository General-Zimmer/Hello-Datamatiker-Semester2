...
public class Opg14 {
    ...
    public static void selectudenparm() {
        try {
            ...
            String sql = "SELECT init, aarstal, plac\n" +
                    "FROM placering";
            ...
            while (res.next()) {
                ...
                int aarstal = res.getInt("aarstal");
                int plac = res.getInt("plac");

                System.out.println(init + "    " + aarstal + "    " + plac);
    ...
    public static void selectmedparm() {
        try {
            ...
            // Laver sql-sætning og får den udført
            String sql = "SELECT init, aarstal, ISNULL(plac, -1) as plac FROM placering where init = '" + inString + "'";
            System.out.println("SQL-streng er "+ sql);
            ResultSet res=stmt.executeQuery(sql);
            //gennemløber svaret
            while (res.next()) {
                ...
                System.out.println(res.getString("init") + "    " + res.getInt("aarstal") + "    " + (plac == -1 ? "UKENDT!" : plac));
            }
            // pæn lukning
            if (!minConnection.isClosed()) minConnection.close();
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            String password="M1a3g2e8";            //skal ændres
