...
public abstract class Controller {
    ...
    public static PN opretPNOrdination(
            ...
            double antal) {
        checkDato(startDato, slutDato);

        PN medicine = new PN(startDato, slutDato, lægemiddel, antal);

        patient.addOrdination(medicine);
        return medicine;
    }
    ...
    public static DagligFast opretDagligFastOrdination(
        ...
        DagligFast medicine = new DagligFast(startDato, slutDato, lægemiddel, morgenAntal, middagAntal, aftenAntal, natAntal);
     ...
     */
    public static DagligSkaev opretDagligSkævOrdination(
            LocalDate startDen, LocalDate slutDen, Patient patient, Lægemiddel lægemiddel,
            LocalTime[] klokkeSlet, double[] antalEnheder) {
        if (klokkeSlet.length != antalEnheder.length) { throw new IllegalArgumentException(); }
        checkDato(startDen, slutDen);

        DagligSkaev ordination = new DagligSkaev(startDen, slutDen, lægemiddel);
    ...
    public static void anvendOrdinationPN(PN ordination, LocalDate dato) {
        LocalDate startDato = ordination.getStartDato().minusDays(1);
        LocalDate slutDato = ordination.getSlutDato().plusDays(1);
    ...
    public static int antalOrdinationerPrVægtPrLægemiddel(
        ...
        for (Patient p : storage.getAllPatienter()) {
            if (vægtStart <= p.getVægt() && p.getVægt() <= vægtSlut) {
                for (Ordination o : p.getOrdinationer()) {
                    if (o.getLaegemiddel() == lægemiddel) {
                        result++;
                    }
                }
            }
    ...
    private static void checkDato(LocalDate startDato, LocalDate slutDato) {
        if (startDato.isAfter(slutDato)) { throw new IllegalArgumentException("SlutDato er før startDato"); }
    }
